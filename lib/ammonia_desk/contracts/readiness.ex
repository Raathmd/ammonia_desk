defmodule AmmoniaDesk.Contracts.Readiness do
  @moduledoc """
  ...
  alias AmmoniaDesk.Contracts.Store
  ...
  def check(product_group) do
    ...
    api_timestamps = AmmoniaDesk.Data.LiveState.last_updated()
  ...
  defp check_expired(issues, active_contracts) do
    expired =
      active_contracts
      |> Enum.filter(&AmmoniaDesk.Contracts.Contract.expired?/1)
  ...
  defp build_report(product_group, active_contracts, all_contracts, api_timestamps, now) do
    %{
      ...
      expired: Enum.count(active_contracts, &AmmoniaDesk.Contracts.Contract.expired?/1),
